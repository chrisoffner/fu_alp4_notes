...
void *Smoker(void *threadid) {
  ...
  for (i = 0; i < 333; i++) {
    ...
    if (*((int *)threadid) == 1) {
      sem_wait(&resource[0]);
      printf("Smoker %d got 0 \n", *((int *)threadid));
      sem_wait(&resource[1]);
      printf("Smoker %d got 1 \n", *((int *)threadid));
    }
    if (*((int *)threadid) == 2) {
      sem_wait(&resource[1]);
      printf("Smoker %d got 1 \n", *((int *)threadid));
      sem_wait(&resource[2]);
      printf("Smoker %d got 2 \n", *((int *)threadid));
    }
    if (*((int *)threadid) == 3) {
      sem_wait(&resource[0]);
      printf("Smoker %d got 0 \n", *((int *)threadid));
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < NUM_THREADS; i++) {
    tids[i] = i;
    if (i == 0)
      rc = pthread_create(&threads[i], NULL, Service, (void *)&tids[i]);
    else
      rc = pthread_create(&threads[i], NULL, Smoker, (void *)&tids[i]);
    if (rc) {
      exit(-1);
    }
  }
